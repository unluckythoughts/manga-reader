...
func GetMangaInfo(t *testing.T, mangaUrl string) (models.Manga, error) {
	...
	assert.Regexp(t, "Infinite Level up in Murim", manga.Title, "manga title mismatch")
	assert.NotEmpty(t, manga.ImageURL, "manga image url is empty")
	assert.NotEmpty(t, manga.Synopsis, "manga synopsis is empty")
	assert.Greater(t, len(manga.Chapters), 60, "could not all the chapters for manga")
...
func TestManga(t *testing.T) {
	...
	url := "https://realmscans.com/series/infinite-level-up-in-murim/"
	GetMangaInfo(t, url)
}
